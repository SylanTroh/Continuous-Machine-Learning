#!/usr/bin/env julia
# Usage: OMP_NUM_THREADS=1 julia --project=. --threads=1 experiments/paired_solver_test.jl
...
const EXPERIMENT = get(ENV, "SOLVER_EXP", "paired_solver_test")
const USE_GPU = get(ENV, "SOLVER_GPU", "auto")
...
const SPLITS = [
    (name="Euler", rep=FUNCTIONREP, backend=:fasteuler),
    (name="RK4", rep=FUNCTIONREP, backend=:fastrk4),
    (name="Tsit5adaptive", rep=FUNCTIONREP, backend=:fastadaptive),
]

const PICARD_SPLIT = (name="Picard", rep=FUNCTIONREP, backend=:picard)
...
const PAIRED_CFG = (; nepochs=NEPOCHS, nsamp=NSAMP, lr=LR, patience=PATIENCE,
    ...
    plateau_evals=PLATEAU_EVALS, min_improve=MIN_IMPROVE, train_win=true)
...
function run_config(cfg, tag; splits=SPLITS, data_to_device, lg)
    ...
    logln(lg, "$(cfg.label): $NSEEDS seeds, backends = $(join(names, ", ")), T=$PRECISION")
    ...
    r = run_split_sweep(task, splits; seeds=SEEDS, hidden=cfg.hidden, ncp=cfg.ncp,
        experiment=EXPERIMENT, tag, data_to_device, lg, per_seed_schedule=true, cfg=PAIRED_CFG,
    ...
    summary = (name=cfg.label, backends=r.names, means=means,
        anova=((NSEEDS >= 2 && length(r.names) >= 2) ? anova_rm(r.errors) : nothing))
    sample_epochs = collect(SAMPLE_EVERY:SAMPLE_EVERY:NEPOCHS)
    meta = (label=cfg.label, hidden=cfg.hidden, ncp=cfg.ncp,
        backends=r.names, euler_steps=euler_steps(cfg.ncp),
    ...
    save_run(path; summary, errors=r.errors, samples=r.samples, epochs=r.epochs,
        backends=r.names, config=meta)
    return summary
end

function make_configs()
    tasks = [("det(3)", () -> DeterminantTask(3)),
        ("MNIST", () -> load_mnist())]
    return [(idx=i, label="$(tname) H$(HIDDEN) N$(NCP)", make_task=mk, hidden=HIDDEN, ncp=NCP)
            for (i, (tname, mk)) in enumerate(tasks)]
end
...
function run_sweep(args)
    ...
    for i in idxs
        run_config(configs[i], "config_$(configs[i].idx)_picard";
            splits=[PICARD_SPLIT], data_to_device, lg)
    end
    close(lg)
end
...
function solver_report(load, lg)
    ...
    for cfg in make_configs()
        ...
        backends = collect(nt.backends)
        errors = [Float64.(e) for e in nt.errors]
        pic = load("config_$(cfg.idx)_picard")
        if pic !== nothing
            n = length(errors[1])
            for (nm, e) in zip(pic.backends, pic.errors)
                (nm in backends || length(e) != n) && continue
                push!(backends, nm)
                push!(errors, Float64.(e))
            end
        end
        _, Y = generate(cfg.make_task(), 10_000; rng=Xoshiro(0))
        Var = sum(target_baseline(Y))
        base = errors[1]
        logln(lg, @sprintf("%s   (Var = %.4g):", nt.summary.name, Var))
        for (nm, e) in zip(backends, errors)
            Êhat = sum(e) / length(e) / Var
            if e === base
                logln(lg, @sprintf("    %-14s Ê = %.4e", nm, Êhat))
            else
                d = (e .- base) ./ Var
                ci = mean_ci(d; level=1 - ALPHA)
                logln(lg, @sprintf("    %-14s Ê = %.4e   ΔÊ = %+.4e   %s%% CI [%+.4e, %+.4e]",
                    nm, Êhat, ci.mean, pct, ci.lo, ci.hi))
            end
        end
        if length(errors[1]) >= 2 && length(errors) >= 2
            a = anova_rm(errors)
