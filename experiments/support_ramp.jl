#!/usr/bin/env julia
# Usage:  OMP_NUM_THREADS=1 julia --project=. --threads=1 experiments/support_ramp.jl
#   (GPU runs REQUIRE --threads=1)
...
const T_DISTS = [
    (:baseline, rng -> 1.0),
    (:uniform, rng -> rand(rng)),
    (:ident2, rng -> rand(rng)^2),
    (:ident3, rng -> rand(rng)^3),
]

const EXPERIMENT = get(ENV, "SR_EXP", "support_ramp")
...
function lerp_sampler(base, n::Int, num_samples::Int, tdist::Function; rng)
    ...
    return () -> begin
        ...
        for _ in 1:num_samples
            t = tdist(rng)
            R = base.input_scale .* randn(rng, n, n)
            M = (1 - t) .* Id .+ t .* R
            push!(inputs, M)
...
function arm_cell(arm::Symbol, tdist::Function, h::Int;
    ...
    logln(lg, "CELL $name: $M_seeds seeds in batches of $seed_batch  (t-dist=$arm)")
    ...
    process = ctx -> begin
        batch = ctx.batch
        models = ctx.models
        gens = [lerp_sampler(_variant_base(variant, n, s), n, num_samples, tdist;
                    rng=Xoshiro(s + DATA_OFFSET)) for s in batch]
        heldout = [generate(_variant_base(variant, n, s), n_fresh; rng=Xoshiro(s + EVAL_OFFSET))
                   for s in batch]
    ...
    logln(lg, @sprintf("Done %s: det E/Var=%.4f±%.4f conv=%d/%d wall=%.0fs",
        name, det_EVar, std(fresh_s; corrected=false), count(conv_s), M_seeds, wall))
    ...
    result = (variant=variant, arm=arm, tag=string(arm), n=n, h=h, ncp=ncp,
        n_params=repr.n_params, threshold=threshold, n_epochs=n_epochs,
...
function support_ramp(; variant::Symbol=VARIANT, dims=DIMS, n::Int=N, t_dists=T_DISTS,
    experiment::String=EXPERIMENT, lg=nothing, kwargs...)
    cells = [() -> arm_cell(name, tdist, h; variant, n, experiment, lg, kwargs...)
             for h in dims for (name, tdist) in t_dists]
    results = run_parallel(cells; max_concurrent=1)
    logln(lg, "Summary (final det-component E/Var on full n×n by t-distribution)")
    for r in results
        logln(lg, @sprintf("  %-9s %-9s n=%d h=%-4d det E/Var=%.4f±%.4f  %s",
            r.variant, r.tag, r.n, r.h, r.det_EVar, r.det_EVar_std,
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    support_ramp(lg=lg; data_to_device=gpu_device(USE_GPU))
    close(lg)
end
